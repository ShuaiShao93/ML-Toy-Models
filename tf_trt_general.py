...
TRT_ENGINE_DIR = OUTPUT_SAVED_MODEL_DIR + "_engine"
...
def input_fn():
    yield [
        tf.zeros(shape=freeze_shape(input_tensor.shape),
...
dataset = tf.data.TFRecordDataset(os.path.join(
    OUTPUT_SAVED_MODEL_DIR, "assets", "trt-serialized-engine.TRTEngineOp_000_000"))
...
raw_engine_path = os.path.join(TRT_ENGINE_DIR, "raw_engine")
if not os.path.exists(TRT_ENGINE_DIR):
    os.mkdir(TRT_ENGINE_DIR)

with open(raw_engine_path, "wb") as f:
    f.write(engine_instance.serialized_engine)

os.system(f"trtexec --loadEngine={raw_engine_path} --dumpProfile")
